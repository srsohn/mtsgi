...
class EnvironmentLoop(core.Worker):
  ...
  def run(
    ...
    while not should_terminate(episode_count, step_count):
      ...
      # TODO: Use option success instead of (reward!=0) here!
  ...
  def run(
    ...
    for trial_idx in range(num_trials):  # for each task
      ...
      for split_iter in tqdm(range(num_trial_splits), desc=f'Task [{trial_idx}/{num_trials}]'):
        ...
        if self._label == 'meta_train' and hasattr(self._test_actor._actor, '_state'):  # RLRL
          self._test_actor._actor._state = self._agent._actor._state
